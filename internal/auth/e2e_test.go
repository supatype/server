...
import (
	...
	"github.com/supatype/server/internal/auth/mailer"
	...
	"github.com/supatype/server/internal/e2e/e2ehooks"
)
...
func getEmailAccessToken(
	...
	return getAccessToken(ctx, t, inst, &auth.PasswordGrantParams{
		Email:    email,
		Password: pass,
	})
}
...
func getAccessToken(
	...
	t *testing.T,
	inst *e2ehooks.Instance,
	req *auth.PasswordGrantParams,
) *auth.AccessTokenResponse {
	res := new(auth.AccessTokenResponse)
...
func TestE2EHooks(t *testing.T) {
		...
		t.Run("AdminEndpoints", func(t *testing.T) {
			...
			t.Run("AdminGenerateLink", func(t *testing.T) {
				...
				t.Run("SignupVerification", func(t *testing.T) {
					...
					email := genEmail()
					req := &auth.GenerateLinkParams{
						Type:     "signup",
						Email:    email,
						Password: "pass1234",
					}
					res := new(auth.GenerateLinkResponse)
	...
	t.Run("MFAVerificationAttempt", func(t *testing.T) {
		...
		type flowResult struct {
			factorRes          *auth.EnrollFactorResponse
			challengeRes       *auth.ChallengeFactorResponse
			mfaUser            *models.User
			mfaUserAccessToken *auth.AccessTokenResponse
		}

		runMFAFlow := func(t *testing.T) *flowResult {
			factorRes := new(auth.EnrollFactorResponse)
			challengeRes := new(auth.ChallengeFactorResponse)
			mfaUser := new(models.User)
			mfaUserAccessToken := new(auth.AccessTokenResponse)
	...
	t.Run("SendEmail", func(t *testing.T) {
		...
		t.Run("SecureEmailChange=Enabled", func(t *testing.T) {
			...
			t.Run("EmailChangeAnonymous", func(t *testing.T) {
				...
				// signup anon
				var accessToken *auth.AccessTokenResponse
				{
					req := &auth.SignupParams{}
					res := new(auth.AccessTokenResponse)
					err := e2eapi.Do(ctx, http.MethodPost, inst.APIServer.URL+"/signup", req, res)
