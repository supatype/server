...
// TestEndToEnd talks to the real Pwned Passwords API.
//
// It has no HTTP stub, so it needs outbound internet and fails on a sandboxed
// or offline runner for reasons that have nothing to do with this package.
// Opt in, the same way the live DNS tests do.
func TestEndToEnd(t *testing.T) {
	if os.Getenv("SUPATYPE_TEST_LIVE_NETWORK") == "" {
		t.Skip("set SUPATYPE_TEST_LIVE_NETWORK=1 to run tests that call the Pwned Passwords API")
	}

	pwnedClient := PwnedClient{
		UserAgent: "tests for https://github.com/supatype/server",
	}
